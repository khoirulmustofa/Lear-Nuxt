...
onMounted(() => {
  const darkTheme = localStorage.getItem('darkTheme') || isDarkTheme
...
onMounted(() => {
  const primary = localStorage.getItem('primary') || 'noir'
  updateColors(
    'primary',
    primaryColors.value.find((c) => c.name === primary)
  )
})

onMounted(() => {
  const surface = localStorage.getItem('surface') || 'slate'
  updateColors(
    'surface',
    surfaces.value.find((c) => c.name === surface)
  )
})

onMounted(() => {
  const mode = localStorage.getItem('menuMode') || layoutConfig.menuMode
  setMenuMode(mode)
  menuMode.value = mode
})
